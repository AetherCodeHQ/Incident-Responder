...
> Incident-Responder - Open source tool by AetherCodeHQ

`cli` `golang` `incident-response` `security`

## What is it?

**Incident Responder** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Incident-Responder --path ./target

# With options
./Incident-Responder --path ./target --format json --output report.json

# Verbose mode
./Incident-Responder --path ./target --verbose
```
...
### Basic scan
```bash
./Incident-Responder --path ./my-project
```

### JSON report
```bash
./Incident-Responder --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Incident Responder
  run: ./Incident-Responder --path . --format json --output report.json
```
